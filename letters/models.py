...
class Letter(models.Model):
    title = models.CharField(max_length=30, unique_for_date="date_time_created")
    content = models.TextField()
    author = models.ForeignKey(User, editable=False)
    show_author = models.BooleanField(default=True, verbose_name='Show the author?')
    date_time_created = models.DateTimeField(auto_now_add=True)
    users_shared_to = models.ManytoManyField(User, default=self.author.last_letter_shared_to(), verbose_name='Addressed to')
...
class UserProfile(models.Model):
    ...
    def last_letter_shared_to(self):
        """Return the last users shared to in a letter"""
        try:
            return self.letter_set.latest().user_shared_to
        except DoesNotExist:
            return None
    ...
    def all_letter_shared_recipients(self):
        """Return all the users shared to or with in letters"""
        return self.letter_set.values('users_shared_to', 'user_shared_with')
        #write function to aggregate these two
